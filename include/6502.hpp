...
namespace NES
{
    ...
    private:
        ...
        uint8_t ASL_val(uint8_t); // for SLO
        ...
        uint8_t LSR_val(uint8_t); // for SRE
        ...
        uint8_t ROL_val(uint8_t);

        // Rotate right
        void ROR(std::function<uint16_t()>);

        uint8_t ROR_val(uint8_t); // for RRA
