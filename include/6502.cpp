...
namespace NES
{
    ...
    void CPU6502::ASL(std::function<uint16_t()> addressing)
    {
        ...
        {
            uint16_t address = addressing();
            uint8_t data = ASL_val(read(address));
            write(address, data);
            tick();
        }
    }

    uint8_t CPU6502::ASL_val(uint8_t data)
    {
    ...
    {
        uint16_t address = addressing();
        uint8_t data = DEC(read(address));
        write(address, data);
    }

    uint8_t CPU6502::DEC(uint8_t data)
    {
        data--;
        setZero(data == 0);
        setNegative(data & 0x80);
    ...
    uint8_t CPU6502::INC(uint8_t data)
    {
        data = data + 1;
        setZero(data == 0);
        setNegative(data & 0x80);
    ...
    uint8_t CPU6502::LSR_val(uint8_t data)
    {
        uint8_t bit0 = data & 1;
        data >>= 1;
        setCarry(bit0);
    ...
    void CPU6502::ROL(std::function<uint16_t()> addressing)
    {
        ...
        {
            uint16_t address = addressing();
            uint8_t data = ROL_val(read(address));
            write(address, data);
            tick();
        }
    }

    uint8_t CPU6502::ROL_val(uint8_t data)
    {
        uint8_t bit7 = data & 0x80;
    ...
    void CPU6502::ROR(std::function<uint16_t()> addressing)
    {
        ...
        {
            uint16_t address = addressing();
            uint8_t data = ROR_val(read(address));
            write(address, data);
            tick();
        }
    }

    uint8_t CPU6502::ROR_val(uint8_t data)
    {
        uint8_t bit0 = data & 1;
    ...
    {
        uint16_t address = addressing();
        uint8_t data = LSR_val(read(address));
        write(address, data);
        EOR(data);
        tick();
    }
